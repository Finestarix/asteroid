...
export default async function updateResetPasswordUser(request: NextApiRequest, response: NextApiResponse) {
    ...
    if (!userData) {
        ...
        try {
            ...
            // @ts-ignore
            data.success = "Successfully changed " + data.data.username + " password to default.";
        } catch (_) {
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            data.error = "Failed to changed " + data.data.name + " password to default.";
        }
    }
